...
/*
        ...
        ver 05    28FEB2007  AJL  Modified to match Zhang Left Hand Coordinates and formatting (see #define ZHANG)
        ver 06    09MAR2007  AJL  Added ALBERTO ascii table format
        ver 07    30APR2012  AJL  Misc bug fixes, and changes to prevent gcc warnings.
                                  Added output of ANU-FMM vgrids.in velocity model file
                                  in current working direcgtory.
...
int VelModToFMM(VelModel* vel_model, GridDesc* grid, FILE* fp_fmmVgridFile, char *waveType);
...
int main(int argc, char *argv[]) {
    ...
    for (nWaveType = 0; nWaveType < NumWaveTypes; nWaveType++) {
        ...
        if ((istat = VelModToFMM(&vel_model, &mod_grid, fp_fmmVgridFile, WaveType[nWaveType])) < 0) {
            nll_puterr("ERROR: loading velocity model to grid.");
            exit(EXIT_ERROR_MODEL);
        }
        ...
        if ((fgets(line, 2 * MAXLINE, inpfile)) &&
            ...
            for (k = 0; k < vel_model->numz; k++) {
                ...
                for (j = 0; j < vel_model->numy; j++) {
                    ...
                    nread = 0;
                    for (i = 0; i < vel_model->numx; i++) {
                        fscanf(inpfile, "%f", &vel_model->array[i][j][k]);
                        nread++;
                    }
                    if (nread < vel_model->numx) {
                        fclose(inpfile);
...
int VelModToFMM(VelModel* vel_model, GridDesc* grid, FILE* fp_fmmVgridFile, char *waveType) {
    ...
    fprintf(fp_fmmVgridFile, "%d %d %d\n", grid->numz, grid->numy, grid->numx);
    ...
    double lon_step = (lon_ur - lon_ll) / (double) (grid->numx - 2);
    ...
    fprintf(fp_fmmVgridFile, "%lf %lf %lf  # z/lat/lon=%f/%f/%f\n", AVG_ERAD - z_orig, lat_orig * cRPD, lon_orig * cRPD, z_orig, lat_orig, lon_orig);
    ...
    /* Note:  staggered grid assumed, thus vel lookup is shifted +dx/2, etc. */
    double xval, yval;
    double zdepth = z_orig;
    int ix, iy, iz;
    double vel;
    for (iz = grid->numz - 1; iz >= 0; iz--) {
        double ylat = lat_orig;
        for (iy = 0; iy < grid->numy; iy++) {
            double xlon = lon_orig;
            for (ix = 0; ix < grid->numx; ix++) {
                if (ModelCoordsMode == COORDS_LATLON) {
                    xval = xlon;
                    yval = ylat;
                ...
                if (vel_model->type == TYPEALBERTO) {
                    vel = vel_model->array[ix][iy][iz]; // NOTE: there must be some error here, since not really working in lat/lon
                } else {
                    vel = get3Dvel(vel_model, xval, yval, zdepth);
                    /*if (imodel >= LAYEROFFSET || imodel < 0) {
                        // check for surface
