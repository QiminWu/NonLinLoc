...
//printf(">%s", line_in);
		...
		if (istat != 14) {
			puterr2("ERROR: reading hypocenter line:", line_in);
			return(-1);
		}
		...
		if ((istat = OpenGrid3dFile(filename_model, &fp_grid,
			...
			puterr2("ERROR: opening model grid files: ", filename_model);
			continue;
		}
		...
		if ((istat = WriteHypocenter(fp_out, &hypo, vel_source, vel_source / VpVsRatio)) < 0)
			puterr("ERROR: writing hypocenter.");
...
//printf("<%s>\n", line_in);
			...
			if (istat != 6) {
				puterr2("ERROR: reading phase:", line_in);
				return(-1);
			}
...
int ReadCalcpd_Input(FILE* fp_input)
{
	...
	while ((fgets_return = fgets(line_in, 4*MAXLINE, fp_input)) != NULL
		...
		if (strcmp(param, "CPDVPVS") == 0)
			if ((istat = get_vp_vs(strchr(line_in, ' '))) < 0)
				puterr("ERROR: reading calcpd Vp/Vs params.");
			else
				flag_vp_vs = 1;
	...
	if (!flag_files)
		puterr("ERROR: no i/o file (CPDFILES) params read.");
	
	if (!flag_vp_vs)
		puterr("INFO: no Vp/Vs (CPDVPVS) params read.");
