...
int ingvList2Hyp(int argc, char *argv[])
{
	...
	if ((fp_hypoell_in = fopen(fn_hypoell_in, "r")) == NULL) {
		puterr("ERROR: opening INGV hypocenter input file.");
		return(-1);
	}
	...
	while (1) {
		...
		if (iformat == 1) {
			if ((istat = ReadINGVBullList(fp_hypoell_in, &Hypo)) == EOF)
				...
				puterr2("ERROR: reading INGV Bull List file", fn_hypoell_in);
				break;
			}
		} else {
			if ((istat = ReadINGVSearch(fp_hypoell_in, &Hypo)) == EOF)
				...
				puterr2("ERROR: reading INGV Search file", fn_hypoell_in);
				break;
			}
		}
		nLocRead++;
