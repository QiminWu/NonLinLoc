...
int surfaceToGrid(struct surface *pgrid2d, GridDesc* pgrid_desc, char *type_str)
{
	...
	if (pgrid_desc->buffer == NULL) {
		puterr("ERROR: allocating memory for 2D temp grid buffer.");
		exit(EXIT_ERROR_MEMORY);
	}
	/* create array access pointers */
	pgrid_desc->array = CreateGridArray(pgrid_desc);
	if (pgrid_desc->array == NULL) {
	puterr("ERROR: creating array for accessing 2D temp grid buffer.");
		exit(EXIT_ERROR_MEMORY);
	}
	...
	while ((fgets_return = fgets(line, 2*MAXLINE, fp_input)) != NULL
		...
		if (strcmp(param, "SGTYPE") == 0)
			if ((istat = get_sg_type(strchr(line, ' '))) < 0) 
				puterr("ERROR: reading Sec2Grid grid type.");
			else
				flag_type = 1;
		...
		if (strcmp(param, "SGSPLINE") == 0)
			if ((istat = get_sg_spline(strchr(line, ' '))) < 0) 
				puterr("ERROR: reading Sec2Grid spline params.");
			else
				flag_spline = 1;
		...
		if (strcmp(param, "SGDISTAZ") == 0)
			if ((istat = get_sg_distaz(strchr(line, ' '))) < 0) 
				puterr("ERROR: reading Sec2Grid distance/azimuth params.");
...
*/
	...
	if (!flag_outfile) 
		puterr("ERROR: no outputfile (SGOUT) params read.");
	if (!flag_type) 
		puterr("ERROR: no type (SGTYPE) params read.");
	if (!flag_spline) 
		puterr("ERROR: no spline (SGSPLINE) params read.");
	if (!flag_grid) 
		puterr("ERROR: no grid (SGGRID) params read.");
	if (!flag_sections) 
		puterr("ERROR: no 2D section (SG2DSECT) params read.");
...
int SetLimits(Sect2D* psection)
{
	...
	if ((fp_mapline = fopen(fn_mapline, "w")) == NULL) {
		puterr("ERROR: opening mapline file.");
	} else {
			...
			for (iz = grid->numz - 1; iz >=0; iz--) {
				...
				switch (grid->type) {
					...
					case GRID_SLOW2_METERS:
					    ...
					    break;
					default:
					     puterr2("ERROR: unrecognized grid type", 
							grid->chr_type);
						return(-1);
				}
...
int mapIsolineCutoff(IsoCut *pisocut, GridDesc* grid, char *fileroot)
{
	...
	if (WriteGrid3dBuf(pdepth_grid, NULL, fileroot, "depth") < 0) {
		puterr("ERROR: writing isoline depth grid to disk.");
		return(-1);
	}
