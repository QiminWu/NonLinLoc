...
int main(int argc, char** argv)
{
	...
	if ((istat = ConvertToCoulomb(argc, argv)) < 0) {
		puterr("ERROR converting to Coulomb grid.");
		exit(-1);
	}
...
int ConvertToCoulomb(int argc, char** argv)
{
	...
	if (coul_grid.buffer == NULL) {
		puterr("ERROR: allocating memory for 3D Coulomb grid buffer.");
		exit(EXIT_ERROR_MEMORY);
	}
	/* create array access pointers */
	coul_grid.array = CreateGridArray(&coul_grid);
	if (coul_grid.array == NULL) {
		puterr("ERROR: creating array for accessing 3D Coulomb grid buffer.");
		exit(EXIT_ERROR_MEMORY);
	}
	// create weights grid
...
//						}
	...
	if (fabs(n_smooth) > 0) {
		...
		if (smoothed_grid.buffer == NULL) {
			puterr("ERROR: allocating memory for 3D Coulomb smooth grid buffer.");
			exit(EXIT_ERROR_MEMORY);
		}
		/* create array access pointers */
		smoothed_grid.array = CreateGridArray(&smoothed_grid);
		if (smoothed_grid.array == NULL) {
			puterr("ERROR: creating array for accessing 3D Coulomb smooth grid buffer.");
			exit(EXIT_ERROR_MEMORY);
		}
