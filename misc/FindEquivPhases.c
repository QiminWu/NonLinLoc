...
int doFindEquivPhases(int argc, char** argv)
{
	...
	if (istat < 0) {
		puterr2("ERROR: opening target NLL hypocenter-phase file.", fn_hyp_target);
		return(-1);
	}
	...
	if (istat < 0) {
		puterr2("ERROR: opening phs_source NLL hypocenter-phase file.", fn_hyp_target);
		return(-1);
	}
...
//printf("MATCHED: dist_min %f <= dist_max_fract * dist_epi %f\n", dist_min, dist_max_fract * dist_epi);
	...
	if (nPhaseMatched) {
		...
		if ((fp_hyp_out = fopen(fn_hyp_out, "w")) == NULL) {
			puterr2("ERROR: opening output NLL phase file", fn_hyp_out);
			return(-1);
		}
